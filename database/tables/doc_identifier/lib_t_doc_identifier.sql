...
COMMENT ON TABLE lib_t_doc_identifier 
IS 'This table contains unique document identifiers. (Type, not instance!)'
/

COMMENT ON COLUMN lib_t_doc_identifier.id 
IS 'Primary key of the table.'
/
COMMENT ON COLUMN lib_t_doc_identifier.doc_identifier_type_id 
IS 'Identifier type (ISBN, ISSN, ...) identifier, foreign key.'
/
COMMENT ON COLUMN lib_t_doc_identifier.content 
IS 'Identifier value.'
/
